...
// Reconnection logic was mainly intended for Docker usage
connectToDB();
...
// Creates entire schema needed for application, mostly used as a guide
async function createTables() {
  ...
  /*
    ...
    [
        {
            type: "product" OR "blend",
            itemid: "1ebc47bf-515c-420c-aec3-71bfa38883d1:" (points to either products table or blends table)
            quantity: 2,    
            item: (snapshot of item's information at time of order)
        },
        {
            type: "product" OR "blend",
            itemid: "83d1420c-aec3-71bfa38883d13e9274:" (points to either products table or blends table)
            quantity: 1,    
            item: (snapshot of item's information at time of order)
        }
    ]
...
/* 
Frequent habits seen in classes:

    - filterableFields are fields that users could "filter" in a class's filter method (such as getFilteredUsers).
    - modifiableFields are fields thay users could "update" in a class's update method (such as updateProduct)"
    - Notice that methods that modify the DB will call themselves again using the "prepareRollback" wrapper if no "client" is being used already.
*/
...
class Users {
  ...
  async updatePreferredNotes(id, preferrednotes, client) {
    if (!client) {
      return prepareRollback((c) =>
        this.updatePreferredNotes(id, preferrednotes, c),
      );
    }
...
class Products {
  ...
  async createProduct(options, client) {
    ...
    const invalidUrls = images.filter(
      (url) => !url.startsWith(CLOUDFRONT_DOMAIN),
    );
  ...
  // Gets all products regardless of hidden of status
  ...
  // Gets all products that are not marked hidden
  ...
  // Gets all products available based on provided filters
  // Builds a query with each field introduced
  async getFilteredProducts(filters, client) {
    ...
    for (const filter of Object.keys(filters)) {
      switch (filter) {
        ...
        case "notes": {
          //checks if any notes in provided list exist under any subfield in "notes" field
        ...
        case "ishidden": {
          // this is a filter for admins only, checked in controllers.js
    ...
    let products;
    try {
      // Gets products based on large filter query
      const res = await client.query(query, values);
      products = res.rows;
    } catch (err) {
      console.error(err);
  ...
  // gets all products with the same parent id
  async getRelatedProducts(parentid, client) {
    ...
    try {
      // Gets products with the parent id
  ...
  // Simply updates review count and the "average review" value (based on a simple formula) when adding a review
  // this is optimal compared to actually fetching all reviews
  async addReview(parentid, rating, client) {
    ...
    try {
      ...
      if (err instanceof DBError) throw err;
      throw new DBError(
        "Failed to update product rating stats when adding rating",
      );
    }
    return { success: true };
  }

  // Simply updates review count and the "average review" value (based on a simple formula) when removing a review
  async removeReview(parentid, rating, client) {
    ...
    try {
      // Update product review info
      // The CASE / WHEN / ELSE syntax is used for conditionals in SQL, in this case to prevent division by 0 issues
      const query = `
            UPDATE products 
            SET review_count = review_count - 1, 
            review_average = CASE 
      ...
      if (err instanceof DBError) throw err;
      throw new DBError(
        "Failed to update product rating stats when removing rating",
      );
    }
    return { success: true };
  }
  ...
  /*
    ...
    Ex) options may be: {type: "10ml spray", price: 35.85}
    */
  ...
  async updateProduct(id, options, client) {
    ...
    try {
      ...
      if (options.images) {
        oldImages = (
          await client.query("SELECT images FROM products WHERE id = $1", [id])
        )?.rows?.[0]?.images;
        if (!oldImages) {
          throw new DBError(
            "Failed to get old images, or product doesn't exist",
          );
        }
      }
      ...
      const updateResult = await client.query(query, [
        ...fields.map((f) =>
          typeof options[f] === "object"
            ? JSON.stringify(options[f])
            : options[f],
        ),
        id,
      ]);
      ...
      if (options.images) {
        // gets new images being added
        const newImages = options.images.filter(
          (url) => !oldImages.includes(url),
        );
        // gets images we are not storing anymore
        const unusedImages = oldImages.filter(
          (url) => !options.images.includes(url),
        );

        // remove tags for the new images
        for (const image of newImages) {
          ...
          try {
            ...
            throw new DBError(
              "Failed to remove temporary tag from uploaded image",
            );
          }
        }
  ...
  async updateProductStock(parentid, stock_ml, client) {
    if (!client) {
      return prepareRollback((c) =>
        this.updateProductStock(parentid, stock_ml, c),
      );
    }

    try {
      // set new stock for all products sharing the given parent id (as they use the same stock)
  ...
  // Mainly used as helper function when canceling orders, it increases, or restores, a product's stock
  async increaseProductStock(id, quantity, client) {
    if (!client) {
      return prepareRollback((c) => this.increaseProductStock(id, quantity, c));
    }
    try {
      // get the product and its size
      const retrieveProductQuery = `SELECT * FROM products WHERE id = $1;`;
      const product = (await client.query(retrieveProductQuery, [id]))
        ?.rows?.[0];
      if (!product) {
        return {
          success: false,
      ...
      //only 40% of size is the stored oil, so add 40% of the size to stock
      const newSize = Number(product.stock_ml) + quantity * size * 0.4;
      if (newSize < 0) {
        return { success: false, message: "Negative new size", status: 400 };
      }

      // update the stock for all products with same parent id
  ...
  async decreaseProductStock(id, quantity, client) {
    ...
    try {
      ...
      //only 40% of size is the stored oil, so remove 40% of the size from stock
      const oilNeeded = quantity * size * 0.4;
      const newSize = Number((Number(product.stock_ml) - oilNeeded).toFixed(2));
      if (newSize < 0) {
      ...
      return {
        success: false,
        message: "Failed to decrease product stock",
        status: 400,
      };
    }
    return { success: true };
  ...
  // Update queries use this special method to generate them since the fields can be anything
...
class Reviews {
  ...
  constructor() {
    // reviews will interact with users and products
  ...
  async createReview(options, client) {
    ...
    const { customerid, productid, message, rating, images, credit_gained } =
      options;
    const id = uuidv4();
    ...
    const updatedProduct = await this.products.addReview(
      productid,
      rating,
      client,
    );
    ...
    try {
      ...
      // add store credit when user creates review that earns it
      if (credit_gained > 0) {
        const addCredit = await this.users.addStoreCredit(
          customerid,
          credit_gained,
          client,
        );
  ...
  async deleteReview(id, client) {
    ...
    try {
      ...
      // reduce store credit when user deletes review that previously earned them store credit
      if (result.credit_gained > 0) {
        const reduceCredit = await this.users.reduceStoreCredit(
          result.customerid,
          result.credit_gained,
          client,
        );
      ...
      // update product rating
      const updatedProduct = await this.products.removeReview(
        result.productid,
        result.rating,
        client,
      );
      ...
      // delete any images in deleted review
  ...
  // gets all reviews for a specific parent id
  async getProductReviews(parentid, client) {
    ...
    let reviews;
    try {
      // get all reviews for a specific parentid, and joins with users to get info of user who created the review
      const query = `
  ...
  async getUserReviews(customerid, client) {
    ...
    // get all reviews for a specific user with "customerid", and joins with users to get info of user who created the review
    const query = `
        SELECT reviews.*, row_to_json(users) AS user
        FROM reviews 
        JOIN users ON reviews.customerid = users.cognito_sub 
  ...
  async getReviews(client) {
    ...
    try {
      // get all review and joins with users to get info of user who created the review
      const query = `
        SELECT reviews.*, row_to_json(users) AS user
        FROM reviews 
        JOIN users ON reviews.customerid = users.cognito_sub 
  ...
  // Used when an admin or user responds to a review
  async respondToReview(id, response, client) {
    ...
    try {
      // Appends the new response to the existing responses list of a review
      ...
      // Only admins may respond as much as they'd like
  ...
  // used to specially format a update query for reviews (fields can include any "allowed to be updated" fields)
  formatUpdateQuery(fields) {
    if (fields.length === 0) return null;
...
class Orders {
  ...
  constructor() {
    // orders are very involved, and interact with users, products, blends, and cartitems
    this.users = new Users();
    this.products = new Products();
  ...
  /*
   Creating an order does many things, including:
        - Gets items in the order from the user's cart
        - decreases stocks for items to be purchased
        - descreases store credit if used
        - stores a snapshot of item ordered, in case it is deleted
*/
  async createOrder(customerid, storeCredit, client) {
    if (!client) {
      return prepareRollback((c) =>
        this.createOrder(customerid, storeCredit, c),
      );
    }
    let order;
    ...
    //Get user's cart
    try {
      const cart = (await this.cartItems.getCart(customerid, client))?.data
        ?.cart;
      if (!cart) {
      ...
      // Adds item price to the total and decreases stock for each cart item
      const items = [];
      let total = 0;
      for (const { itemid, quantity, type, item } of cart) {
        ...
        if (type === "product") {
          ...
          const result = await this.products.decreaseProductStock(
            itemid,
            quantity,
            client,
          );
          ...
          const result = await this.blends.decreaseBlendStock(
            itemid,
            quantity,
            client,
          );
      ...
      // If the user is using store credit, reduce their store credit
      if (storeCredit > 0) {
        const reduceCredit = await this.users.reduceStoreCredit(
          customerid,
          Math.min(storeCredit, total),
          client,
        );
        if (!reduceCredit?.success) {
          throw new DBError(reduceCredit.message);
  ...
  //Should probably only be for development, as it is important to track all orders
  async deleteOrder(id, client) {
    ...
    try {
      ...
      if (order.status !== "canceled" && order.status !== "fulfilled") {
        const cancelOrderRes = await this.cancelOrder(
          id,
          "dev reasons",
          client,
        );
        if (!cancelOrderRes.success) {
          throw new DBError(
            "Failed to cancel order / restore stock before deletion",
          );
        }
      }
  ...
  // Treated as the inverse of create order (as delete order is only really for development).
  /*
   Canceling an order does many things, including:
        - Increases / restores stocks for items purchased
        - Increases / restores store credit if used
*/
  async cancelOrder(id, cancelreason, client) {
    ...
    try {
      // update order status to be canceled
      ...
      // if store credit was used, restore it
      ...
      // Restore the stock of each item in the canceled order
      ...
      for (const { itemid, quantity, type } of order.items) {
        if (type === "product") {
          const result = await this.products.increaseProductStock(
            itemid,
            quantity,
            client,
          );
          ...
          const result = await this.blends.increaseBlendStock(
            itemid,
            quantity,
            client,
          );
          if (
            !result.success &&
            result.message !== "Blend does not exist" &&
  ...
  // Flow is ususally pending -> mixing -> ready ->  fulfilled, but may move backwards
  ...
  async getOrder(id, client) {
    ...
    let order;
    try {
      // Fetched orders are joined with users to have them include user emails
      // Gets a specific order with the users email included
      const query = `
      ...
      // adds blends names for convenience
  ...
  // gets all orders belonging to a user that were made in the past 7 days
  async getUserRecentOrders(customerid, client) {
    if (!client) {
      return prepareRollback((c) => this.getUserRecentOrders(customerid, c));
    }

    // Gets all orders (up to 10) for a user that were made in the past 7 days. (with user emails included )
    const query = `
        SELECT orders.*, users.email
        FROM orders 
        JOIN users ON orders.customerid = users.cognito_sub 
  ...
  // similar to getFilteredProducts, but for orders
  // builds a large query to filter for orders
  async getFilteredOrders(filters, client) {
    ...
    // initital query. Joins with users to include user emails
    let query = `
        SELECT orders.*, users.email
        FROM orders 
        JOIN users ON orders.customerid = users.cognito_sub 
    ...
    let orders;
    try {
      // gets orders based on filter query
      const res = await client.query(query, values);
      orders = res.rows;
    } catch (err) {
      console.error(err);
  ...
  // adds blend names for convenience
...
class Blends {
  ...
  async getBlendById(blendid, client) {
    ...
    try {
      ...
      return {
        success: false,
        message: "Failed to retrieve blend",
        status: 500,
      };
    }
  }
  ...
  async increaseBlendStock(id, quantity, client) {
    ...
    try {
      ...
      const productsUsed = (
        await client.query(getProductsUsedQuery, [productIds])
      )?.rows;

      if (
        !productsUsed ||
        (blend.frag3_productid && productsUsed.length !== 3) ||
        (!blend.frag3_productid && productsUsed.length !== 2)
      ) {
        throw new DBError("Failed to retrieve products"); //Do not change error message
      }

      const newProductSizes = [
        Number(productsUsed[0].stock_ml) + quantity * oil * blend.frag1_pct,
        Number(productsUsed[1].stock_ml) + quantity * oil * blend.frag2_pct,
      ];
      if (blend.frag3_productid)
        newProductSizes.push(
          Number(productsUsed[2].stock_ml) + quantity * oil * blend.frag3_pct,
        );
      ...
      const product1UpdateRes = (
        await client.query(productUpdateQuery, [
          newProductSizes[0],
          productsUsed[0].parentid,
        ])
      )?.rows?.[0];
      if (!product1UpdateRes)
        throw new DBError(
          `Failed to update blend product 1 with parentid ${productsUsed[0].parentid}`,
        );
      const product2UpdateRes = (
        await client.query(productUpdateQuery, [
          newProductSizes[1],
          productsUsed[1].parentid,
        ])
      )?.rows?.[0];
      if (!product2UpdateRes)
        throw new DBError(
          `Failed to update blend product 2 with parentid ${productsUsed[1].parentid}`,
        );
      if (blend.frag3_productid) {
        const product3UpdateRes = (
          await client.query(productUpdateQuery, [
            newProductSizes[2],
            productsUsed[2].parentid,
          ])
        )?.rows?.[0];
        if (!product3UpdateRes)
          throw new DBError(
            `Failed to update blend product 3 with parentid ${productsUsed[2].parentid}`,
          );
      }

      const pricePerML1 =
        Number(productsUsed[0].price) /
        Number(productsUsed[0].variation.split("ml")?.[0]);
      price += pricePerML1 * (blend.size_ml * blend.frag1_pct);

      const pricePerML2 =
        Number(productsUsed[1].price) /
        Number(productsUsed[1].variation.split("ml")?.[0]);
      price += pricePerML2 * (blend.size_ml * blend.frag2_pct);

      if (blend.frag3_productid) {
        const pricePerML3 =
          Number(productsUsed[2].price) /
          Number(productsUsed[2].variation.split("ml")?.[0]);
      ...
      return {
        success: false,
        message: "Failed to increase blend stock",
        status: 400,
      };
    }
  ...
  async decreaseBlendStock(id, quantity, client) {
    ...
    try {
      ...
      if (!product1UpdateRes)
        throw new DBError(
          `Failed to update blend product 1 with id ${blend.frag1_productid}`,
        );
      const product2UpdateRes = (
        await client.query(productUpdateQuery, [
          newProductSizes[1],
      ...
      if (!product2UpdateRes)
        throw new DBError(
          `Failed to update blend product 2 with id ${blend.frag2_productid}`,
        );
      if (blend.frag3_productid) {
        const product3UpdateRes = (
          await client.query(productUpdateQuery, [
        ...
        if (!product3UpdateRes)
          throw new DBError(
            `Failed to update blend product 3 with id ${blend.frag3_productid}`,
          );
      }
      ...
      return {
        success: false,
        message: "Failed to decrease blend stock",
        status: 400,
      };
    }
    return { success: true, data: { price } };
  ...
  validateBlendInput(options) {
    ...
    if (!frag1_productid || !frag2_productid)
      throw new DBError("At least 2 fragrances are required");
    if (frag1_productid === frag2_productid)
      throw new DBError("Fragrance 1 and 2 cannot be the same product");
    if (![30, 50].includes(Number(size_ml)))
      throw new DBError("Bottle size must be 30 or 50 ml");

    const hasThird = !!frag3_productid;
    if (hasThird && (frag3_pct === null || frag3_pct === undefined))
      throw new DBError("3rd fragrance percentage is required");
    if (!hasThird && frag3_pct)
      throw new DBError(
        "3rd fragrance product is required when a percentage is provided",
      );

    const total =
      Number(frag1_pct) +
      Number(frag2_pct) +
      (hasThird ? Number(frag3_pct) : 0);
    if (total !== 100)
      throw new DBError(
        `Fragrance percentages must add up to 100%, currently: ${total}%`,
      );
  }
}

class CartItems {
  // creates a specific cart item
  ...
  async createCartItem(options, client) {
    ...
    if (type === "blend") {
      ...
      const total_oil_ml = Number(blend.size_ml) * 0.4;
      const fragsToCheck = [
      ...
      for (const frag of fragsToCheck) {
        ...
        try {
          const res = await client.query(
            `SELECT id, name, stock_ml FROM products WHERE id = $1;`,
            [frag.productid],
          );
          product = res.rows?.[0];
        } catch (err) {
        ...
        if (!product)
          throw new DBError(`Product not found: ${frag.productid}`, 404);
  ...
  async deleteCartItem(id, client) {
    ...
    try {
      ...
      if (err instanceof DBError) throw err;
      throw new DBError(
        "Failed to either delete cart item or decrement quantity",
      );
    }
    return { success: true };
  }
  ...
  /*
    ...
    Also, gets entire cart of a user
     */
    ...
    try {
      ...
      for (const item of items) {
        if (
          ...
          throw new DBError(
            "Invalid cart item. A quantity, item id, and type is required",
          );
        }
        const id = uuidv4();
...
// saved items is VERY similar to cart items
class SavedItems {
  ...
  async createSavedItem(options, client) {
    ...
    return {
      success: true,
      data: { exists: false, savedItem: result.rows[0] },
    };
  }
  ...
  async updateSavedItems(customerid, items, client) {
    if (!client) {
      return prepareRollback((c) =>
        this.updateSavedItems(customerid, items, c),
      );
    }
    const result = [];
    try {
      ...
      for (const item of items) {
        if (!item.itemid || !item.type) {
          throw new DBError(
            "Invalid saved item. An item id and type is required",
          );
        }
        const id = uuidv4();
  ...
  async getRecommendations(userid, client) {
    ...
    try {
      ...
      const getProductNotes = async (productid) => {
        ...
        try {
          const res = await client.query(
            `SELECT notes FROM products WHERE id = $1;`,
            [productid],
          );
      ...
      try {
        const userRes = await client.query(
          `SELECT favorite_notes FROM users WHERE cognito_sub = $1;`,
          [userid],
        );
        const favNotesRaw = userRes.rows?.[0]?.favorite_notes || "";
        const favNotes = favNotesRaw
          .split(",")
          .map((n) => n.trim())
          .filter(Boolean);
      ...
      const candidatesRes = await client.query(
        `SELECT * FROM products WHERE ishidden = false;`,
      );
      const candidates = candidatesRes.rows.filter(
        (p) => !seenProductIds.has(p.id),
      );
