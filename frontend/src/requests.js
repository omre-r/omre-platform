// The base url used for all requests in this file
const backendURL = "http://localhost:5001";

// Note that each request basically maps to a function in controllers.js
// Ex: getUsersReq maps to getUsers in controllers.js

/*
Almost all requests use this exact format:

        const response = await fetch(backendURL + `/someurl`, {
...
// General error wrapper to ensure no function in this file crashes
...
async function getFilteredUsersReq(filters) {
  const response = await fetch(
    backendURL + `/users/filter?filters=${JSON.stringify(filters)}`,
    {
      headers: { Authorization: `Bearer ${getToken()}` },
    },
  );
...
async function reduceStoreCreditReq(id, credit) {
  const response = await fetch(
    backendURL + `/users/${id}/store_credit/reduce`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
...
async function getFilteredProductsReq(filters) {
  const response = await fetch(
    backendURL + `/products/filter?filters=${JSON.stringify(filters)}`,
    {
      headers: { Authorization: `Bearer ${getToken()}` },
    },
  );
...
async function createReviewReq({
  customerid,
  productid,
  message,
  rating,
...
async function getUserRecentOrdersReq(customerid) {
  const response = await fetch(
    backendURL + `/orders/user/${customerid}/recent`,
    {
      headers: { Authorization: `Bearer ${getToken()}` },
    },
  );
...
async function getFilteredOrdersReq(filters) {
  const response = await fetch(
    backendURL + `/orders/filter?filters=${JSON.stringify(filters)}`,
    {
      headers: { Authorization: `Bearer ${getToken()}` },
    },
  );
...
const ALLOWED_IMAGE_TYPES = [
  ...
  "image/png",
  "image/webp",
]; //We are allowing upto 4 images types
...
async function getPresignedUrlReq_LOCAL(file, bucketDirectory) {
  const response = await fetch(
    backendURL +
      `/uploadurl?filename=${file.name}&contentType=${file.type}&fileSize=${file.size}&bucketDirectory=${bucketDirectory}`,
    {
      headers: { Authorization: `Bearer ${getToken()}` },
    },
  );
...
async function uploadAndGetURlsReq(imageFiles, bucketDirectory) {
  ...
  const uploadUrls = await Promise.all(
    imageFiles.map((f) => getPresignedUrlReq_LOCAL(f, bucketDirectory)),
  );
  const uploadResults = await Promise.all(
    uploadUrls.map((data, i) =>
      uploadImageToS3Req(data.uploadUrl, imageFiles[i]),
    ),
  );
...
async function createProductFlowReq_LOCAL({
  parentid = null,
  type,
  name,
  variation,
  ...
  const uploadUrls = await Promise.all(
    images.map((f) => getPresignedUrlReq_LOCAL(f, "products")),
  );
  if (uploadUrls.some((url) => url === null))
    throw new Error("Failed to get upload URLs");

  const uploadResults = await Promise.all(
    uploadUrls.map((data, i) => uploadImageToS3Req(data.uploadUrl, images[i])),
  );
  if (uploadResults.some((res) => res === null))
  ...
  return await createProductReq({
    ...
    variation,
    price,
    images: uploadUrls.map((data) => data.publicUrl),
    stock_ml,
    notes,
    description,
    isfeatured,
...
async function getPresignedUrlReq(file) {
  const response = await fetch(
    "https://vsazml20a1.execute-api.us-east-1.amazonaws.com/prod/products/presigned-url",
    {
      method: "POST",
...
async function createProductAWSReq(productData) {
  const response = await fetch(
    "https://vsazml20a1.execute-api.us-east-1.amazonaws.com/prod/products",
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function createProductAWSFlowReq(
  { name, price, description, imageFiles, mainImageIndex = 0, notes },
  onProgress = null,
) {
  // <--- DELETED "export"
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    if (onProgress)
      onProgress(`Uploading image ${i + 1} of ${imageFiles.length}...`);
...
// Manually wrapping each function in the error wrapper since JavaScript does not have decorators
...
export {
  ...
  updateLastLoginReq,
  getFilteredUsersReq,
  updatePreferredNotesReq,
  addStoreCreditReq,
  reduceStoreCreditReq,
  getUserRecentOrdersReq,
  getProductReq,
